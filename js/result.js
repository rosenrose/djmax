...
const coverMulti3XY = [[276, 120], [866, 121], [866, 563]];
...
document.querySelectorAll("#details, #hide, #coverNum").forEach(input => {input.dispatchEvent(new InputEvent("change"));});
...
function writeResult() {
    if (select == "inputSingle") {
        ...
        for (let i=0; i<files.length; i++) {
            ...
            promises.push(new Promise(resolve => {
                image.onload = () => {
                    ...
                    ctx.drawImage(targetImage, cropSingleX, cropSingleY, width, cropSingleHeight, drawX, drawY, width, cropSingleHeight);
                    if (is_detail) {
                        ctx.drawImage(targetImage, cropDetailsX, cropDetailsY, cropDetailsWidth, cropDetailsHeight, drawX+cropSingleWidth, drawY+detailsOffsetY, cropDetailsWidth, cropDetailsHeight);
                    }
                    resolve();
                };
            }));
        ...
        image.onload = () => {
            if (image.width == baseWidth && image.height == baseHeight) {
                ctx.drawImage(image, cropMultiX, cropMultiY, cropMultiWidth, cropMultiHeight, 0, 0, cropMultiWidth, cropMultiHeight);
            }
            else {
                resizeCtx.drawImage(image, 0, 0, image.width, image.height, 0, 0, baseWidth, baseHeight);
                ctx.drawImage(resizeCanvas, cropMultiX, cropMultiY, cropMultiWidth, cropMultiHeight, 0, 0, cropMultiWidth, cropMultiHeight);
            }
            if (is_cover) {
                writeCover();
            }
...
function writeCover() {
    switch (select) {
        case "inputSingle":
            ...
            if (files.length >= 3) {
                if (files.length == 3) {
                    drawCover(coverSingleX + width/2, coverSingleY + cropSingleHeight, coverWidth, coverHeight);
                }
                else {
...
function drawWatermark() {
    ...
    let [textWidth, textHeight] = [Math.round(ctx.measureText(watermark).width), parseInt(ctx.font.match(/\d+/)[0])];
    let margin = 5;
