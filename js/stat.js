...
function App() {
    React.useEffect(() => {
        ...
        .then(json => {
            ...
            for (let category in list["songs"]) {
                let catDat = {"category": category, "levelSum": {}, "patternCount": {}, "levelAvg": {}, "noteSum": {}, "noteAvg": {}};

                commonHeads.forEach(head => {
                    catDat["levelSum"][head] = catDat["patternCount"][head] = catDat["levelAvg"][head] = catDat["noteSum"][head] = catDat["noteAvg"][head] = 0;
                });
                ...
                for (let song of list["songs"][category]) {
                    ...
                    if (!(patternCountList.find(pc => pc["patternCount"] == song["patternCount"]["전체"]))) {
                        patternCountList.push({"patternCount": song["patternCount"]["전체"], "전체": 0});
                    }
    ...
    const onBpmChange = (event) => {
        ...
        if (th.dataset.sorted) {
            sortTitle({ mode, "order": th.dataset.sorted, "bpmMode": event.target.value });
    ...
    const onDateChange = (event) => {
        ...
        let th = event.target.closest("th");
        if (th.dataset.sorted) {
            sortTitle({ mode, "order": th.dataset.sorted, "dateMode": event.target.value });
            setSort(cur => (cur == ASC)? DES : ASC);
        }
    };
    ...
    const onThClick = (event) => {
        sortTable({ "target": event.target, mode, titleMode, bpmMode, dateMode, isLevelsNotesSort, setSort });
    };
...
function Select({ onChange }) {
    return (
        <div id="selectMode" onChange={onChange}>
            <div>
                <label><input type="radio" name="mode" value="levelAvg" defaultChecked/>레벨 평균</label>
                ...
                <label><input type="radio" name="mode" value="levelHistogram"/>레벨 분포</label>
            </div>
            <div>
                ...
                <label><input type="radio" name="mode" value="patternCountHistogram"/>패턴 개수 분포</label>
            </div>
            <div>
...
function Thead({ mode, titleMode, bpmMode, dateMode, isLevelsNotesSort, btnSelect, rankSelect, onTitleChange, onBpmChange, onDateChange, onIsLevelsNotesSort, onBtnSelectChange, onRankSelectChange, onThClick }) {
    React.useEffect(() => {
    ...
    switch (mode) {
        case "levelHistogram":
            leftTopTh = <th className="click">레벨<span className="sortArrow">{ASC}</span></th>
            break;
        case "patternCountHistogram":
            leftTopTh = <th className="click">패턴 개수<span className="sortArrow">{ASC}</span></th>
            break;
        case "noteHistogram":
            leftTopTh = <th className="click">노트 수<span className="sortArrow">{ASC}</span></th>
            break;
        default:
            ...
            >
                ...
                {(mode == "bpm" && head == "BPM") &&
                    <div className="subSelectMode" onChange={onBpmChange}>
                        <label><input type="radio" name="bpmMode" value="min" defaultChecked={bpmMode == "min"}/>최솟값</label>
                        <label><input type="radio" name="bpmMode" value="mid" defaultChecked={bpmMode == "mid"}/>중간값</label>
                        <label><input type="radio" name="bpmMode" value="max" defaultChecked={bpmMode == "max"}/>최댓값</label>
                    </div>
                }
                {(mode == "date" && head == "수록 날짜") &&
                    <div className="subSelectMode" onChange={onDateChange}>
                        <label><input type="radio" name="dateMode" value="early" defaultChecked={dateMode == "early"}/>빠른 날짜</label>
                        <label><input type="radio" name="dateMode" value="PS4" defaultChecked={dateMode == "PS4"}/>PS4</label>
                ...
                <tr className="subSelectMode btnSelect" onChange={onBtnSelectChange}>
                    {commonHeads.slice(1).map(btn =>
                        <th key={btn} className={btnSelect.has(btn)? `${btn}-background btn-rank` : `${btn}-color`}>
                            <label>
                                {isLevelsNotesSort && <input type="checkbox" value={btn} defaultChecked={btnSelect.has(btn)} className="block"/>}
                                {btn}
                            </label>
                        </th>
                    )}
                ...
                <tr className="subSelectMode rankSelect" onChange={onRankSelectChange}>
                    {ranks.map(rank =>
                        <th key={rank} className={(!isLevelsNotesSort || rankSelect.has(rank))? `${rank}-background btn-rank` : `${rank}-color`}>
                            <label>
                                {isLevelsNotesSort && <input type="checkbox" value={rank} defaultChecked={rankSelect.has(rank)} className="block"/>}
                                {rank}
                            </label>
                        </th>
                    )}
                    ...
                    switch (mode) {
                        ...
                        case "bpm":
                            data = (((bpmMode == "min")
                                        ...
                                        : (bpmMode == "mid")
                                            ? totalBpmMid
                                            : totalBpmMax) / songs.length).toFixed(1);
                            break;
                        case "minLevel":
...
function sortTable({ target, mode, titleMode, bpmMode, dateMode, isLevelsNotesSort, setSort }) {
    ...
    if ((target.matches("label") && checked === false) || //label 클릭시 제목/카테고리 전환할 때
    ...
    // console.log(head);
    if (th.matches(".categorySort") || label?.matches(".categorySort")) {   //오름차순 정렬만
        document.querySelector("th[data-sorted]")?.removeAttribute("data-sorted");
        th.dataset.sorted = ASC;
    }
    else {
        if (th.dataset.sorted) {    //정렬 전환
...
function sortTitle({ mode, head, order, bpmMode, dateMode }) {
    if (head == "제목") {
        ...
        if (mode == "length") {
            ...
            songs.sort((a,b) => {
                if (categoryList.indexOf(a["category"]) == categoryList.indexOf(b["category"])) {
                    if (a["date"][dateMode] - b["date"][dateMode] == 0) {   //a["date"][dateMode].valueOf() == b[dateMode]["PC"].valueOf()
                        let prev = document.querySelector("#titleLabel span").textContent;
                        return (
                            (prev == ASC)
                                ? a["title"].toLowerCase().localeCompare(b["title"].toLowerCase())
                    ...
                    return (
                        (prev == ASC)? a["date"][dateMode] - b["date"][dateMode] : b["date"][dateMode] - a["date"][dateMode]
                    );
                }
                return categoryList.indexOf(a["category"]) - categoryList.indexOf(b["category"]);
            });
        ...
        switch (mode) {
            ...
            case "date":
                songs.sort((a,b) => {
                    ...
                    return (
                        (order == ASC)? a["date"][dateMode] - b["date"][dateMode] : b["date"][dateMode] - a["date"][dateMode]
                    );
                });
                break;
        }
...
function Graph({ mode, tbodyMode }) {
    ...
    React.useEffect(() => { //렌더링이 끝난 후 호출됨. Real DOM에 접근 가능.
...
function drawGraph({ mode, tbodyMode, head, x, y, text, orientation, height, xrange, yrange, margin }) {
    ...
    Plotly.newPlot(`graph-${mode}-${tbodyMode}-${head}`, data, layout, {staticPlot: true, responsive: true});
}
