...
.then(json => {
    ...
    document.querySelector("#selectMode").addEventListener("change", event => {
        ...
        selectAttribute(`#${(mode != "histogram")? "stat" : "histogram"}`, "hidden", false, true, ...document.querySelectorAll("#stat, #histogram"));
        selectAttribute(`.${mode}`, "hidden", false, true, ...document.querySelectorAll("#graph > div"))

        if (mode != "histogram") {
            selectAttribute(`.${mode}`, "hidden", false, true, ...table.querySelectorAll("tr:not(.firstColumn):not(.levelsSortTr)"));
            toggleAttribute("hidden", !["levels","notes"].includes(mode), ...titleTbody.querySelectorAll("img"));
            toggleAttribute("rowSpan", ["levels","notes"].includes(mode)? 5 : 2, ...titleTbody.querySelectorAll("th.title"));

            thead.querySelector("#titleDiv").style.display = (mode != "patternCountAvg")? "" : "none";
            thead.querySelector("#categoryDiv").style.display = (["levelAvg","patternCount","patternCountAvg"].includes(mode))? "" : "none";
            toggleAttribute("hidden", !["levelAvg","patternCount"].includes(mode), thead.querySelector("#seperator"), titleBtn, categoryBtn);

            thead.querySelector("th").rowSpan = ["levels","notes"].includes(mode)? 3 : 2;
            if (mode == "levels") {
                thead.querySelector("#levelsSort input").dispatchEvent(new InputEvent("change",{bubbles: true}));
            }
            else {
                levelsTbody.hidden = true;
            }

            toggleAttribute("hidden", ["patternCountAvg","levels","notes"].includes(mode), ...tfoot.querySelectorAll(`.avg.firstColumn, .avg.${mode}`));
            toggleAttribute("hidden", !["patternCount","length"].includes(mode), ...tfoot.querySelectorAll(`.sum.firstColumn, .sum.${mode}`));
            toggleAttribute("hidden", mode != "patternCount", ...tfoot.querySelectorAll(`.ratio.firstColumn, .ratio.${mode}`));

            if (["levelAvg","patternCount"].includes(mode)) {
                ((titleBtn.checked)? titleBtn : categoryBtn).dispatchEvent(new InputEvent("change",{bubbles: true}));
            }
            else if (mode != "levels") {
                ((mode == "patternCountAvg")? categoryBtn : titleBtn).dispatchEvent(new InputEvent("change",{bubbles: true}));
            }
        }
    });
    document.querySelectorAll("input[checked]").forEach(input => {input.dispatchEvent(new InputEvent("change",{bubbles: true}));});
    document.querySelector("input[type='checkbox']").dispatchEvent(new InputEvent("change",{bubbles: true}));
...
function tableInit() {
    thead.querySelector("th").addEventListener("change", event => {
        ...
        selectAttribute(`#${(titleMode == "title")? "titleTbody" : "categoryTbody"}`, "hidden", false, true, titleTbody, categoryTbody);

        toggleAttribute("hidden", !(mode == "patternCount" && titleMode == "category"), ...table.querySelectorAll(".SCpercent"));

        tfoot.hidden = ["patternCountAvg","date","levels","notes"].includes(mode) || (mode == "levelAvg" && titleMode == "category");
        if (mode == "patternCount") {
            selectAttribute(`.${(titleMode == "title")? "title" : "category"}`, "hidden", false, true, ...tfoot.querySelectorAll(".avg.patternCount"));
        }

        document.querySelector("#graph .levelAvg").hidden = !(mode == "levelAvg" && titleMode == "category");
    })
    ...
    // levelsTbody에 tr 생성
    for (let i=0; i<Math.max(totalPatternCount["4B"],totalPatternCount["5B"],totalPatternCount["6B"],totalPatternCount["8B"]); i++) {
    ...
    for (let mode in tableHeads) {
        ...
        tableHeads[mode].forEach((head, i) => {
            ...
            let isSortClick = !["length","levels","notes"].includes(mode) ||
                                (mode == "length" && head == "길이") ||
    ...
    thead.querySelector("#levelsSort").addEventListener("change", event => {
        if (event.target.matches("[type='checkbox']")) {
            ...
            selectAttribute(`#${isLevelsSort? "levelsTbody" : "titleTbody"}`, "hidden", false, true, titleTbody, levelsTbody);
        }
        else {
...
function patternCount() {
    ...
    writePatternCount();
    tfoot.querySelector(".avg.patternCount.title").querySelectorAll("th").forEach((th, i) => {
        th.textContent = (totalPatternCount[tableHeads["patternCount"][i]] / songs.length).toFixed(1);
    });
    tfoot.querySelector(".avg.patternCount.category").querySelectorAll("th").forEach((th, i) => {
        th.textContent = (totalPatternCount[tableHeads["patternCount"][i]] / categoryData.length).toFixed(1);
    });
    tfoot.querySelector(".sum.patternCount").querySelectorAll("th").forEach((th, i) => {
        th.textContent = totalPatternCount[tableHeads["patternCount"][i]];
    });
    tfoot.querySelector(".ratio.patternCount").querySelectorAll("th").forEach((th, i) => {
        if (i>0) {
            let ratio = (totalPatternCount[tableHeads["patternCount"][i]] / totalPatternCount["전체"])*100;
...
function patternCountAvg() {
    ...
    let categoryDataSorted = [...categoryData].sort((a,b) => categoryList.indexOf(a["category"]) - categoryList.indexOf(b["category"]));
    drawGraph("patternCountAvg",
        categoryList.map(category => list["dlcKor"][category]),
        commonHeads.map(head => categoryDataSorted.map(catDat => catDat["patternCountAvg"][head].toFixed(2)))
    );
}
...
function length() {
    ...
    writeLength();
    tfoot.querySelector(".avg.length th").textContent = `${parseInt(avgLength/60)}분 ${avgLength%60}초`;
    let minutes = totalLength % 3600;
    tfoot.querySelector(".sum.length th").textContent = `${parseInt(totalLength/3600)}시간 ${parseInt(minutes/60)}분 ${minutes%60}초`;
}
...
function writeDate() {
    titleTbody.querySelectorAll("tr.date").forEach((tr, i) => {
        ...
        tds[1].style.backgroundImage = getCategoryStyle(category, list["collaboration"]).backgroundImage;
    });
}
function date() {
...
function writeLevels() {
    ...
    titleTbody.querySelectorAll("tr.levels").forEach((tr, i) => {
        tr.querySelectorAll("td:not([rowspan])").forEach((td, j) => {
            let rank = ranks[j];
            writeLevelsTd(rank, songs[parseInt(i/4)]["level"][btns[i%4]][rank], rank in songs[parseInt(i/4)]["level"][btns[i%4]], td);
        })
        if (i%4 == 0) {
            tr.querySelector("td[rowspan]").textContent = list["dlcKor"][songs[parseInt(i/4)]["category"]];
        }
    });
}
function writeLevelsSort() {
    levelsTbody.querySelectorAll("tr").forEach((tr, i) => {
        ...
        if (i < levelList[btn].length) {
            ...
            tr.querySelectorAll("td").forEach((td, j) => {
                writeLevelsTd(ranks[j], levelList[btn][i]["level"], levelList[btn][i]["rank"] == ranks[j], td);
            });
        }
    });
}
...
function writeNotes() {
    ...
    titleTbody.querySelectorAll("tr.notes").forEach((tr, i) => {
        tr.querySelectorAll("td:not([rowspan])").forEach((td, j) => {
            let rank = ranks[j];
            td.textContent = (rank in songs[parseInt(i/4)]["note"][btns[i%4]])? songs[parseInt(i/4)]["note"][btns[i%4]][rank] : "　";
        })
        if (i%4 == 0) {
            tr.querySelector("td[rowspan]").textContent = list["dlcKor"][songs[parseInt(i/4)]["category"]];
        }
